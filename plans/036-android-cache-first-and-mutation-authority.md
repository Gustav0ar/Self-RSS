...
PR #67 first CI JVM job reports one older LocalStore test failure among 582 cases. Its immediate Paging invalidation assertion assumes synchronous Room notification. The test now awaits the actual registered invalidation callback, matching the repository contract already used elsewhere. Unit failures now upload the XML/HTML reports because the original job retained only a coroutine wrapper line, not the exact assertion. This changes test synchronization and diagnostics only; the original CI assertion cannot be identified conclusively without that missing report. Scope includes that existing test and the unit-report artifact step in Android CI.
