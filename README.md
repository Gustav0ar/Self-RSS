...
To host the app on your VPS behind Traefik:

1. **GitHub Repository Secrets:**
   Configure the following secrets in your GitHub repository (`Settings > Secrets and variables > Actions`):
   - `VPS_HOST`: The IP address or domain name of your VPS.
   - `VPS_USERNAME`: The user to SSH as (e.g., `root` or `ubuntu`).
   - `VPS_SSH_KEY`: The private SSH key used to access your VPS.
   - `VPS_PORT`: Optional, port (defaults to `22`).

2. **Server Directory Layout:**
   Create a folder `/opt/self-feed` on the VPS to store your configurations and copy [docker-compose.yml](docker-compose.yml) into it:
   ```bash
   mkdir -p /opt/self-feed
   # Copy docker-compose.yml to /opt/self-feed/docker-compose.yml
   ```

3. **Configure Environment Variables:**
    Create a `.env` file at `/opt/self-feed/.env` with the following variables:
    ```env
    DOMAIN_NAME=selffeed.yourdomain.com
    IMAGE_OWNER_LOWERCASE=your-github-owner-or-org
    REDIS_PASSWORD=use-a-strong-random-redis-password
    JWT_SECRET=use-a-long-random-string-min-32chars
    JWT_REFRESH_SECRET=use-another-different-long-random-string-min-32chars
    CORS_ALLOWED_ORIGINS=https://selffeed.yourdomain.com
    ALLOW_REGISTRATION=true     # Set to 'false' once you register your account
    ADMIN_EMAIL=admin@yourdomain.com
    ADMIN_PASSWORD=use-a-secure-admin-password
    ```

4. **GitHub Packages Registry Login:**
   Since your repository is private, the VPS needs read access to your GHCR images. Log in manually on the VPS once:
   ```bash
   echo "YOUR_GITHUB_PAT" | docker login ghcr.io -u YOUR_GITHUB_USERNAME --password-stdin
   ```
   *(Note: The PAT requires the `read:packages` scope).*

5. **Deploy & Start:**
   Make sure Traefik is running and the external network `traefik_public` exists:
   ```bash
