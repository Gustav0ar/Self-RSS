...
- API integration tests (temporary SQLite database + ephemeral Redis): `bun run test:integration`
...
- Android (not in Bun workspace): `./packages/android/gradlew -p packages/android :app:testDeviceTestUnitTest` (unit),
  `./packages/android/gradlew -p packages/android :app:connectedDeviceTestAndroidTest` (instrumentation),
  `./packages/android/gradlew -p packages/android :app:lintDebug :app:assembleDebug`

## High-level architecture
- Monorepo: `packages/shared` (contracts + Zod), `packages/api` (Bun + Hono + Drizzle + SQLite/Redis), `packages/web` (React 19 + TanStack Router/Query), `packages/android` (Jetpack Compose client).
...
- API startup (`packages/api/src/index.ts`) loads env, runs SQLite migrations, connects Redis, builds deps, and starts Hono. Scheduled feed sync, enrichment, cache warming, session cleanup, and retention cleanup run in the separate `packages/api/src/worker.ts` process.
...
- Article search uses SQLite FTS5 (`articles_fts`).
