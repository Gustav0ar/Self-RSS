...
export function searchArticles(
	...
	const params: (string | number)[] = [ftsQuery, userId, userId, userId, ...feedIds];
...
function searchSelectSql() {
	return 'WITH fts AS (SELECT article_id, bm25(articles_fts) AS fts_rank FROM articles_fts WHERE articles_fts MATCH ?) SELECT a.id, a.feed_id as feedId, a.canonical_url as canonicalUrl, a.title, a.author, a.excerpt, a.hero_image_url as heroImageUrl, a.published_at as publishedAt, a.fetched_at as fetchedAt, a.content_status as contentStatus, a.content_version as contentVersion, f.title as feedTitle, f.favicon_url as feedFaviconUrl, ar.user_id IS NOT NULL as isRead, article_saves.user_id IS NOT NULL as isSaved, coalesce(aus.read_revision, 0) as readRevision, coalesce(aus.saved_revision, 0) as savedRevision, fts.fts_rank as ftsRank FROM articles a INNER JOIN feeds f ON a.feed_id = f.id INNER JOIN fts ON a.id = fts.article_id LEFT JOIN article_reads ar ON a.id = ar.article_id AND ar.user_id = ? LEFT JOIN article_saves ON a.id = article_saves.article_id AND article_saves.user_id = ? LEFT JOIN article_user_states aus ON a.id = aus.article_id AND aus.user_id = ?';
}
